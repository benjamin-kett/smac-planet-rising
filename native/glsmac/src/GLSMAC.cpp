...
#ifdef __APPLE__
	// Cocoa's main thread outlives the worker threads. Join them while their
	// shared script objects are still alive, after frontend requests are canceled.
	g_engine->StopWorkerThreads();
#endif
			...
			NATIVE_CALL( this ) {
				...
				if ( !arguments.empty() ) {
					...
					if ( !m_state->m_slots->GetSlots().empty() ) {
						GSE_ERROR( gse::EC.GAME_ERROR, "Faction selection requires a new single-player game" );
					}
					faction = m_state->GetFM()->Get( faction_id );
					if ( !faction ) {
						GSE_ERROR( gse::EC.GAME_ERROR, "Unknown faction: " + faction_id );
					}
				}
				if ( m_state->m_slots->GetSlots().empty() ) {
...
#ifdef __APPLE__
	platform::PlayMacIntro( g_engine->GetConfig()->GetPrefix() + "openingx.mp4" );
#endif
...
void GLSMAC::AddSinglePlayerSlot( game::backend::faction::Faction* const faction ) {
	...
	NEWV( player, ::game::backend::Player,
		...
		faction,
		rules.m_difficulty_levels.GetString( m_state->m_settings.global.difficulty_level )
	);
